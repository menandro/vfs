...
__global__ void TgvConvertDisparityToDepthKannalaKernel(float* disparity, float baseline,
...
void StereoTgv::ConvertDisparityToDepthKannala(float* disparity, float baseline, float focal, 
	int w, int h, int s, float* depth)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));

	TgvConvertDisparityToDepthKannalaKernel << <blocks, threads >> > (disparity, baseline, focal, w, h, s, depth);
}
